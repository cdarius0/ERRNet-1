...
def paired_data_transforms(img_1, img_2, unaligned_transforms=False):
    ...
    target_size = int(random.randint(224, 224) / 2.) * 2
    ...
    if random.random() < 0.5:
        img_2 = F.hflip(img_2)
    i, j, h, w = get_params(img_1, (224, 432))
...
class CEILDataset(BaseDataset):
    def __init__(self, datadir,  name_of_experiment, fns=None, size=None, enable_transforms=True, low_sigma=2,
                 high_sigma=5, low_gamma=1.3,
                 high_gamma=1.3, min_opacity=0.8, max_opacity=1.0, min_back_opacity=1.0, max_back_opacity=1.0,
                 kernel_sizes=None, current_epoch=-1, vertical_flip_percentage=None, **kwargs):
        super(CEILDataset, self).__init__()
        self.size = size
        self.datadir = datadir
        self.enable_transforms = enable_transforms
        self.vertical_flip_percentage = vertical_flip_percentage
        sortkey = lambda key: os.path.split(key)[-1]
        self.paths = sorted(make_dataset(datadir, fns), key=sortkey)
        if size is not None:
            self.paths = self.paths[:size]

        self.syn_model = ReflectionSythesis_1(low_sigma=low_sigma, high_sigma=high_sigma, low_gamma=low_gamma,
                                              high_gamma=high_gamma,
                                              min_opacity=min_opacity, max_opacity=max_opacity,
                                              min_back_opacity=min_back_opacity,
                                              max_back_opacity=max_back_opacity, kernel_sizes=kernel_sizes)

        file_name = './checkpoints/' + name_of_experiment + '/reflection_synthesis_parameters_options'
        now = (str(datetime.datetime.now().replace(second=0, microsecond=0))).replace(':', '_').replace(' ', '.')[:-3]
        with open(file_name,
                  'a+') as syntetical_reflection_opt:  # `a+` create file if it does not exist and append to it
            syntetical_reflection_opt.write("-------------Epoch {epoch} {now}-------------\n"
                                            "kernel_sizes: {kernel_sizes}\n"
                                            "sigma: {low_sigma}->{high_sigma}\n"
                                            "gamma: {low_gamma}->{high_gamma}\n"
                                            "trans_layer_opacity: {min_back_opacity}->{max_back_opacity}\n"
                                            "refl_layer_opacity: {min_opacity}->{max_opacity}"
                                            "\nvertical_flip_percentage: {v_flip}\n"
                                            .format(now=now, epoch=current_epoch, low_sigma=low_sigma,
                                                    high_sigma=high_sigma,
                                                    low_gamma=low_gamma, high_gamma=high_gamma,
                                                    min_opacity=min_opacity, max_opacity=max_opacity,
                                                    min_back_opacity=min_back_opacity,
                                                    max_back_opacity=max_back_opacity,
                                                    kernel_sizes=kernel_sizes, v_flip=vertical_flip_percentage))
            for key, value in kwargs.items():
                syntetical_reflection_opt.write("{0}: {1}\n".format(key, value))
            options_file = './checkpoints/'+name_of_experiment+'/opt.txt'
            with open(options_file,
                      'a') as opt_file:
                opt_file.write("/\\-------------Epoch {epoch} {now}-------------/\\ \n\n\n".format(epoch=current_epoch, now=now))
    ...
    def data_synthesis(self, t_img, r_img):
        ...
        # decide to flip image if kernel_size=1, dont flip if kernel_size=2
        ...
        # import scipy.misc
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/4/' + str(self.count) + 'target_.bmp', t_img)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/4/'+str(self.count)+'residual_.bmp', r_img)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/4/' + str(self.count) + 'input_.bmp', m_img)
        # self.count += 1
    ...
    def __getitem__(self, index):
        ...
        # img = transforms.ToPILImage()(M)
        # img.save('/media/Data/darius/ERRNet_new_pull/ERRNet/checkpoints/Big_Dataset_small_images_change_reflection/web/testing_reflection_generator_function'
        #              '/images_saved_from_reflect_gen_function_1/testing/line_155_reflect_dataset/index_'+str(index)+'.bmp')
        # m = transforms.ToPILImage(M)
        from util.util import tensor2im
        # m = tensor2im(m).astype(np.uint8)
        # m_image = Image.fromarray(m)
        # m_image.save('/media/Data/darius/ERRNet_new_pull/ERRNet/checkpoints/Big_Dataset_small_images_change_reflection/web/testing_reflection_generator_function'
        #              '/images_saved_from_reflect_gen_function_1/testing/line_158_reflect_dataset/index_'+index+'.bmp')
...
class CEILTestDataset(BaseDataset):
    def __init__(self, datadir, fns=None, fnstrans_layer=None, size=None, enable_transforms=False,
                 unaligned_transforms=False, round_factor=1, flag=None):
        super(CEILTestDataset, self).__init__()
        self.size = size
        self.datadir = datadir
        # self.fns = fns or os.listdir(join(datadir, 'blended'))
        # self.fnstrans_layer = fnstrans_layer
        # use this fns for barney images
    ...
    def __getitem__(self, index):
        fn = self.fns[index]
        # fntr = self.fnstrans_layer[index]

        # t_img = Image.open(join(self.datadir, 'transmission_layer', fntr)).convert('RGB')
        # m_img = Image.open(join(self.datadir, 'blended', fn)).convert('RGB')

        # use these 2 for barney images
        t_img = Image.open(join(self.datadir, fn)).convert('RGB')
...
class CEILTestDataset_darius(BaseDataset):
    def __init__(self, datadir, datadir_syn=None, fns=None, fns_syn=None, size=None, enable_transforms=True,
                 low_sigma=2, high_sigma=5, low_gamma=1.3, high_gamma=1.3, opacity=0.1):
        super(CEILTestDataset_darius, self).__init__()
        self.size = size
        self.datadir = datadir
        self.datadir_syn = datadir_syn
        self.enable_transforms = enable_transforms
        sortkey = lambda key: os.path.split(key)[-1]
        self.paths = sorted(make_dataset(datadir, fns), key=sortkey)
        self.paths_syn = sorted(make_dataset(datadir_syn, fns_syn), key=sortkey)
        if size is not None:
            self.paths = self.paths[:size]

        # self.syn_model = ReflectionSythesis_1(kernel_sizes=[3], low_sigma=low_sigma, high_sigma=high_sigma,
        #                                       low_gamma=low_gamma, high_gamma=high_gamma, opacity=opacity)
    ...
    def reset(self, shuffle=False):
        ...
        self.B_paths = self.paths[0:num_paths]
        self.R_paths = self.paths_syn[0:num_paths]
