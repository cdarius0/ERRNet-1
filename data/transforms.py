...
class ReflectionSythesis_1(object):
    ...
    def __init__(self, kernel_sizes=None, low_sigma=2, high_sigma=5, low_gamma=1.3, high_gamma=1.3, min_opacity=0.8,
                 max_opacity=1.0, min_back_opacity=1.0, max_back_opacity=1.0):

        self.kernel_sizes = kernel_sizes or [1]  # should be odd but not necessary.
        ...
        self.min_back_opacity = min_back_opacity
        self.max_back_opacity = max_back_opacity
        # self.count = 0
    ...
    def __call__(self, B, R):
        ...
        rand = random.uniform(self.min_opacity, self.max_opacity)
        reflection_opacity = rand

        rand = random.uniform(self.min_back_opacity, self.max_back_opacity)
        background_opacity = rand
        ...
        if kernel_size == 1:
            R = F.vflip(R)
        elif kernel_size == 3 and random.random() < 0.5:
            R = F.vflip(R)
        sigma = np.random.uniform(self.low_sigma, self.high_sigma) #standard deviation
        ...
        R_blur = R_
        # kernel = cv2.getGaussianKernel(11, sigma)   # (11,sigma) - the first param is the kernel of gaussian (should be odd) and sigma is standard deviation.
        kernel = cv2.getGaussianKernel(kernel_size, sigma)
        kernel2d = np.dot(kernel, kernel.T)

        # comment this for no blurr
        for i in range(3):
            R_blur[..., i] = convolve2d(R_blur[..., i], kernel2d, mode='same')

        M_ = background_opacity * B_ + reflection_opacity * R_blur

        if np.max(M_) > 1.0:
            m = M_[M_ > 1]
            m = (np.mean(m) - 1) * gamma
            R_blur = np.clip(reflection_opacity * R_blur - m, 0, 1)
            M_ = np.clip(background_opacity * B_ + reflection_opacity * R_blur, 0, 1)

        # import scipy.misc
        # dest = '/media/Data/darius/ERRNet_new_pull/ERRNet/checkpoints/Big_Dataset_small_images_change_reflection/web/testing_reflection_generator_function/' \
        #        'images_saved_from_reflect_gen_function_1/'+'testing/'#'kernel_size_'+str(self.kernel_sizes[0])+'_sig_'+str(sigma)+'_gamma_'+str(gamma)+'/'
        # if not os.path.isdir(dest):
        #     os.mkdir(dest)
        # scipy.misc.imsave(dest + str(self.count) + '_1gt' + '.bmp', B_)
        # scipy.misc.imsave(dest + str(self.count) + '_2reflection' + '.bmp', R_blur)
        # scipy.misc.imsave(dest + str(self.count) + '_0mixed' + '.bmp', M_)

        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/gt_' + str(self.count) + '_.bmp', B_)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/reflex_'+str(self.count)+'_blurr3_opac1.0.bmp', R_blur)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/blended_' + str(self.count) + '_blurr3_opac1.0.bmp', B_ + R_blur)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/reflex_'+str(self.count)+'_blurr3_opac0.5.bmp', 0.5 * R_blur)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/blended_' + str(self.count) + '_blurr3_opac0.5.bmp', B_ + 0.5 * R_blur)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/reflex_'+str(self.count)+'_blurr3_opac0.75.bmp', 0.75 * R_blur)
        # scipy.misc.imsave('/media/Data/darius/residual_testing_to_remove_blurring/3/blended_' + str(self.count) + '_blurr3_opac0.75.bmp', B_ + 0.75 * R_blur)
        # self.count += 1
