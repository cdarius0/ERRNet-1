    ...
    def train(self, train_loader, **kwargs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if not opt.no_log:
                ...
                if iterations % opt.display_freq == 0 and opt.display_id != 0 or epoch_iteration < 3:
                    save_result = iterations % opt.update_html_freq == 0
                    self.visualizer.display_current_results(model.get_current_visuals(), epoch, epoch_iteration,
                                                            save_result, batch_size=opt.batchSize)
                    # save iteration images
                if iterations % opt.print_freq == 0 and opt.display_id != 0:
                    t = (time.time() - iter_start_time)

                # if epoch_iteration % 2000 == 0:
                #         print('saving the model at epoch %d, iteration %d' %
                #                 (self.epoch, epoch_iteration))
                #         model.save()
                #
                #         print('saving the latest model at the end of epoch %d, iteration %d' %
                #               (self.epoch, epoch_iteration))
                #         model.save(label='latest')

            epoch_iteration += 1
            self.iterations += 1  # this is where you should save if you want to save after x iterations but keep in mind
                                  # that self.iterations is total iterations so use self.iterations % (epoch *number of iterations per epoch(23393))
        self.epoch += 1
        epoch_iteration = 0

        if not self.opt.no_log:
            if self.epoch % opt.save_epoch_freq == 0:
                print('saving the model at epoch %d, total iterations %d' %
                      (self.epoch, self.iterations))
                model.save()

            print('saving the latest model at the end of epoch %d, total iterations %d' %
                  (self.epoch, self.iterations))
            model.save(label='latest')
