...
class ERRNetBase(BaseModel):
            ...
            if savedir is not None:
                if self.data_name is not None:
                    ...
                    if suffix is not None:
                        Image.fromarray(output_i.astype(np.uint8)).save(join(savedir, name,'{}_{}.bmp'.format(self.opt.name, suffix)))
                    else:
                        Image.fromarray(output_i.astype(np.uint8)).save(join(savedir, name, 'output.bmp'))
                    Image.fromarray(target.astype(np.uint8)).save(join(savedir, name, 't_label.bmp'))
                    Image.fromarray(tensor2im(self.input).astype(np.uint8)).save(join(savedir, name, 'm_input.bmp'))
                else:
                    ...
                    Image.fromarray(target.astype(np.uint8)).save(join(savedir, 'transmission_layer', str(self._count)+'.bmp'))
                    Image.fromarray(tensor2im(self.input).astype(np.uint8)).save(join(savedir, 'blended', str(self._count)+'.bmpF'))
                    self._count += 1
    ...
    def test(self, data, savedir=None):
        ...
        if self.data_name is not None and savedir is not None:
            ...
            if os.path.exists(join(savedir, name, '{}.bmp'.format(self.opt.name))):
                return 
        ...
        with torch.no_grad():
            ...
            if self.data_name is not None and savedir is not None:                                          #save output as png
                Image.fromarray(tensor2im(output_i).astype(np.uint8)).save(join(savedir, name, 'output.bmp'.format(self.opt.name)))
                Image.fromarray(tensor2im(self.input).astype(np.uint8)).save(join(savedir, name, 'm_input.bmp'))
                Image.fromarray(tensor2im(self.input - output_i).astype(np.uint8)).save(join(savedir, name, 'residual.bmp'))
...
class ERRNetModel(ERRNetBase):
    ...
    def initialize(self, opt):
        ...
        if opt.resume and os.path.isfile(opt.checkpoints_dir + '/' + opt.name + '/' + str(opt.resume_epoch)+'.pt'):
            self.load(self, opt.resume_epoch)
        # resume from latest epoch by default if resume option is true and no epoch is specified
        elif opt.resume and opt.resume_epoch is None:
            self.load(self, 'errnet_latest')
    ...
    def get_current_visuals(self):
        ...
        ret_visuals['i_output'] = tensor2im(self.output_i).astype(np.uint8)
        ret_visuals['i_target'] = tensor2im(self.target_t).astype(np.uint8)
        ret_visuals['residual'] = tensor2im((self.input - self.output_i)).astype(np.uint8)
        ret_visuals['residual_with_gt'] = tensor2im((self.input - self.target_t)).astype(np.uint8)  # residual compared with the groundtruth
        return ret_visuals       

    @staticmethod
    def load(model, resume_epoch=None):  # load model from resume_epoch
        icnn_path = model.opt.checkpoints_dir + '/' + model.opt.name + '/'\
                    +str(resume_epoch)+'.pt'  # path where checkpoints are saved depending on model name
        if not os.path.isfile(icnn_path):
            icnn_path = None
            print('There is no errnet_latest.pt file to load.')
        state_dict = None
